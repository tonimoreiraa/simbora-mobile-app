...
const fetchAddresses = async () =>
  (await api.get<Address[]>('/user-addresses')).data;

export function UserAddresses() {
  const {data, isLoading, isSuccess} = useQuery('@addresses', fetchAddresses);
    ...
    <SafeAreaView style={tw`flex-1`}>
      <View style={tw`px-4`}>
        ...
        {isSuccess && !data?.length && (
          <Text style={tw`text-sm text-stone-600 text-center my-10`}>
        ...
        {data?.map(address => (
          <View
            key={address.id}
            style={tw`bg-stone-200 px-4 py-3 flex-row rounded-lg`}>
            <BouncyCheckbox size={16} fillColor="#000" />
            <View>
        ...
        <TouchableOpacity
          ...
          <Text style={tw`text-lg text-white`}>Continuar</Text>
        </TouchableOpacity>
      </View>
      <NewAddressDrawer />
    </SafeAreaView>
