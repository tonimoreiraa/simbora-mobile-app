      ...
      <View style={tw`flex-row justify-between items-center mt-4 mb-2`}>
        ...
        <TouchableOpacity>
          <Text style={tw`text-gray-500 text-base`}>
            Ver todas <Text style={tw`text-lg`}>›</Text>
          </Text>
        </TouchableOpacity>
      </View>
        ...
        {isLoading ? (
          [...Array(8).keys()].map(index => (
            <View style={tw`p-1 w-1/4`} key={`skeleton-${index}`}>
              <View
                style={tw`bg-neutral-200 rounded-3xl aspect-square w-full`}
              />
            </View>
          ))
          ...
          categoriesToShow.map(category => (
            <View style={tw`p-1 w-1/4`} key={`category-${category.id}`}>
              <TouchableOpacity>
                <View
                  style={tw`rounded-3xl aspect-square w-full overflow-hidden relative`}>
                  <Image
