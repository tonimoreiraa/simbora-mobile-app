      ...
      <ScrollView style={tw`h-full`}>
        ...
        {data && data.total > data.orders.length && (
          <View style={tw`flex-row justify-center items-center py-4`}>
            <Text 
              style={tw`${page > 1 ? 'text-orange-500' : 'text-gray-300'} px-4 py-2`}
            ...
            <Text 
              style={tw`${page < Math.ceil(data.total / 10) ? 'text-orange-500' : 'text-gray-300'} px-4 py-2`}
              onPress={() => page < Math.ceil(data.total / 10) && setPage(p => p + 1)}
            >
              Próxima
            </Text>
          </View>
